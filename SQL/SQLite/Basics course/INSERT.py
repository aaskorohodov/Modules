...
def write_to_base():
    # первый способ добавить данные в базу:
    cur.execute('''
    INSERT INTO users(userid, fname, lname, gender) 
       VALUES
       ('00001', 'Alex', 'Smith', 'male');
        ''')
    conn.commit()

    # второй способ:
    user = ('00002', 'Lois', 'Lane', 'Female')  # кортеж
    cur.execute("INSERT INTO users VALUES(?, ?, ?, ?);", user)
    conn.commit()


    # третий способ (сразу несколько записей):
    more_users = [('00003', 'Peter', 'Parker', 'Male'), ('00004', 'Bruce', 'Wayne', 'male')]  # передается список кортежей
    cur.executemany("INSERT INTO users VALUES(?, ?, ?, ?);", more_users)
    conn.commit()
    '''Тут используется метод executemany, который умеет работать с несколькими кортежами
    * не пытайся повторно вызвать функцию, так как перезапись не случится, вылезет ошибка, ввиду повторяющегося 
    primary key у повторной записи'''


def more_writing_to_base():
    customers = [
